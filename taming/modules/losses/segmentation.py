...
class CELovaszSoftmaxWithQuant(nn.Module):
    def __init__(self, codebook_weight=1.0, lovasz_weight=1.0, ce_weight=1.0):
        super().__init__()
        self.codebook_weight = codebook_weight
    ...
    def forward(self, qloss, target, prediction, split):
        ...
        probs = F.softmax(prediction, dim=1)
        lovasz_loss = CELovaszSoftmaxWithQuant.lovasz_softmax(probs, target_indices, per_image=False, ignore=None)

        loss = (self.ce_weight * ce_loss) + (self.lovasz_weight * lovasz_loss) + (self.codebook_weight * qloss)

        log_dict = {
            f"{split}/total_loss": loss.clone().detach().mean(),
            f"{split}/ce_loss": ce_loss.detach().mean(),
            f"{split}/lovasz_loss": lovasz_loss.detach().mean(),
            f"{split}/quant_loss": qloss.detach().mean()
        }
        return loss, log_dict
    ...
    def lovasz_softmax(probas, labels, classes='present', per_image=False, ignore=None):
        ...
        """
        if per_image:
            loss = CELovaszSoftmaxWithQuant.mean(CELovaszSoftmaxWithQuant.lovasz_softmax_flat(*CELovaszSoftmaxWithQuant.flatten_probas(prob.unsqueeze(0), lab.unsqueeze(0), ignore), classes=classes)
                              for prob, lab in zip(probas, labels))
        else:
            loss = CELovaszSoftmaxWithQuant.lovasz_softmax_flat(*CELovaszSoftmaxWithQuant.flatten_probas(probas, labels, ignore), classes=classes)
        return loss
    ...
    def lovasz_softmax_flat(probas, labels, classes='present'):
        ...
        for c in class_to_sum:
            ...
            perm = perm.data
            fg_sorted = fg[perm]
            losses.append(torch.dot(errors_sorted, Variable(CELovaszSoftmaxWithQuant.lovasz_grad(fg_sorted))))
        return CELovaszSoftmaxWithQuant.mean(losses)
    ...
    def mean(l, ignore_nan=False, empty=0):
        ...
        l = iter(l)
        if ignore_nan:
            l = ifilterfalse(CELovaszSoftmaxWithQuant.isnan, l)
        try:
            n = 1
            acc = next(l)
